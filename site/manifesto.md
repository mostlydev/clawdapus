...
6. **Compute Is a Privilege** -- Every cognitive cycle is an authorized expenditure. The operator assigns models and schedules; the proxy meters usage and exposes the policy surface for budget controls. The bot does not choose its own budget.
...
Clawdapus injects compiled contract context into shared proxy mounts and resolves caller identity from bearer tokens (`<agent-id>:<secret>`). Because it is context-aware, the proxy acts as a dynamic governance enforcement point. It can drop specific managed tool calls based on the agent's identity. Furthermore, because it acts as the central router, it meters compute usage and can silently downgrade a requested model (e.g., from `gpt-4o` to `claude-3-haiku`). Hard budget caps and proxy-level rate limits are the next enforcement step built on this metering surface.
...
By default, Clawdapus deploys shared governance proxy service(s) per pod, named by type (`cllama-<type>`). It uses the Bearer Token to resolve which agent is making the request, dynamically loads that agent's specific contract, and applies the policy. This drastically reduces resource overhead for multi-agent fleets while enabling pod-wide compute telemetry and policy controls.

**Compiled Tool Mediation:**
Services declare callable tools in their `claw.describe` descriptor. `claw up` compiles per-agent `tools.json` from the policy-filtered tool catalog — deny by default. At runtime, cllama injects the compiled schemas into each upstream LLM request, intercepts `tool_call` responses, executes them against the service, and loops until the model returns terminal text. The runner receives only the final response. Managed tool rounds are fully transparent to the runner and appear as normal completions. Cross-turn continuity is maintained via keepalive during long tool executions. See [ADR-020](https://github.com/mostlydev/clawdapus/blob/master/docs/decisions/020-cllama-compiled-tool-mediation.md).
...
The **ambient memory plane** is live and builds on these surfaces. Pluggable memory services consume the session history ledger and derive durable state — facts, commitments, episodic summaries, project context. The governance proxy recalls that state into future inference turns automatically, before each upstream inference request. Memory recall is query-aware: unlike feeds, which deliver the same cached content regardless of conversation, recall is shaped by the current inference request. Memory intelligence — embeddings, ranking, summarization, graph extraction — lives in swappable services behind a stable contract, not in the proxy or the runner. `claw memory backfill` replays the immutable ledger into a new memory service; `claw memory forget` tombstones entries without mutating the ledger. See [ADR-021](https://github.com/mostlydev/clawdapus/blob/master/docs/decisions/021-memory-plane-and-pluggable-recall.md).
...
- **The Governance Proxy is its Sensory Organ:** The shared `cllama` proxy is a credential firewall and telemetry choke point. It meters usage, applies compiled model/tool boundaries, and emits telemetry logs. It does not "think" about fleet management.
...
**Why extend Docker?**
The hard problems of containerization are solved. Extending Docker inherits decades of infrastructure investment, layer caching, and CI/CD compatibility.

**Why do Clawfiles compile to Dockerfiles?**
Because no custom build engine survives contact with production. `claw build` is a preprocessor. The output is a standard Dockerfile. `docker build` does the rest.

**Why is the behavioral contract a bind mount?**
Purpose changes faster than infrastructure. The bind mount means the operator changes purpose at the speed of editing a text file, and it keeps the purpose outside the blast radius of a compromised container.

**Why are Claws users, not privileged processes?**
Every service already has an authorization model. While the governance proxy acts as a "cognitive intent gate" (proactively dropping LLM tool requests that violate policy), the target service remains the "execution gate" for all network traffic. Give the Claw credentials via standard env vars, and let the service's own auth decide what those credentials allow. The proxy governs what the agent is allowed to *think about doing*; the service governs what it is allowed to *actually do*.

**Why are lightweight runners first-class?**
Because governance is not proportional to complexity. A 400-line Python script with brokerage API access needs the exact same purpose contract and governance proxy as a massive agent OS.
