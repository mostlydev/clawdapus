...
5. **Surfaces Are Declared** — Bots communicate through shared surfaces (volumes, APIs). Surfaces serve two audiences: operators get topology visibility, bots get capability discovery. While Docker enforces physical access modes on mounts, the governance proxy enforces API permissions by proactively scoping and intercepting unauthorized tool calls.
6. **Claws Are Users** — A Claw authenticates to external services with standard credentials (environment variables). The governance proxy acts as an intelligent authorization layer that can proactively deny tool calls before they are made, but the target service's own auth still governs the final execution.
...
**Why are Claws users, not privileged processes?** Every service already has an authorization model. While the governance proxy acts as an "intent gate" (proactively dropping tool calls that violate policy), the target service acts as the "execution gate." Give the Claw credentials via standard env vars, and let the service's own auth decide what those credentials allow. The proxy governs what the agent is allowed to *try*; the service governs what it is allowed to *do*.
