...
func TestComposeEffectiveAgentsMD(t *testing.T) {
	md := ComposeEffectiveAgentsMD("# Contract\n\n", "# CLAWDAPUS.md\n\nruntime facts\n")

	if !strings.HasPrefix(md, "# Contract\n\n--- BEGIN: infrastructure_context (guide) ---") {
		t.Fatalf("expected raw contract followed by infrastructure block, got:\n%s", md)
	}
	for _, want := range []string{
		"This infrastructure context was generated by Clawdapus.",
		"It is also available at `/workspace/CLAWDAPUS.md`.",
		"# CLAWDAPUS.md",
		"runtime facts",
		"--- END: infrastructure_context (guide) ---\n",
	} {
		if !strings.Contains(md, want) {
			t.Fatalf("expected effective contract to contain %q, got:\n%s", want, md)
		}
	}
}
