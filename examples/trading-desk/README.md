...
| File | Purpose |
|------|---------|
| `Clawfile` | OpenClaw trader image (desk-manager, momentum-trader) |
| `Clawfile.nanoclaw` | NanoClaw image (systems-monitor) |
| `Clawfile.nullclaw` | NullClaw image with Discord startup announcement (value-trader) |
| `Clawfile.microclaw` | MicroClaw spike image with Discord startup announcement |
| `Clawfile.hermes` | Hermes spike image with Discord startup announcement |
...
```
DESK_MANAGER_BOT_TOKEN=Bot ...
MOMENTUM_TRADER_BOT_TOKEN=Bot ...
SYSTEMS_MONITOR_BOT_TOKEN=Bot ...
VALUE_TRADER_BOT_TOKEN=Bot ...
MICRO_BOT_TOKEN=Bot ...
...
**Requirements:**
- Docker running
- Env-owned Discord IDs for desk-manager, momentum-trader, systems-monitor, value-trader, micro, and hermes, plus shared guild/channel IDs
- Real bot tokens in `examples/trading-desk/.env` (`DESK_MANAGER_BOT_TOKEN` at minimum; others can reuse it for local spike runs)
...
The test skips automatically if `DESK_MANAGER_BOT_TOKEN` is not set in `.env`, or if
