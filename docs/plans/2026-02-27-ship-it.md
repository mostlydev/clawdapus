...
The pod-file-missing test (line 67-80) checks for `"run 'claw compose up %s' first"` — this DOES change because Step 1 updated the source to say `"run 'claw up %s' first"`. Update the assertion:

```go
// Before:
if !strings.Contains(got, "run 'claw compose up") {
// After:
if !strings.Contains(got, "run 'claw up") {
```

If the existing assertion doesn't match either pattern exactly, read the test file and update whatever string check references "claw compose up" to "claw up".
