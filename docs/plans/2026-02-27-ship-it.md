...
**Architecture:** Flatten compose-namespaced lifecycle commands to top-level `claw *`, add Telegram/Slack handle support to match OpenClaw's native channels, build a `claw init --from` migration command that scaffolds from existing OpenClaw configs, ship a hello-world quickstart example, and publish prebuilt binaries via GoReleaser + a curl-friendly install script.
...
**ADR:** [`docs/decisions/010-cli-surface-simplification.md`](../decisions/010-cli-surface-simplification.md) — rationale for this change.
...
The `composeCmd` Cobra parent goes away. Each subcommand registers directly on `rootCmd`. The `-f` flag moves to each command individually (or a shared persistent flag on root). Error messages should use top-level `claw up`.
...
The `resolveComposeGeneratedPath` default-missing error message doesn't mention "compose" — it says "rerun from pod directory or pass --file". So no assertion change needed for that test.

The pod-file-missing test (line 67-80) should check for `"run 'claw up %s' first"`. Update the assertion:

```go
if !strings.Contains(got, "run 'claw up") {
```

If the existing assertion doesn't match exactly, update the string check to the top-level command wording.
...
git commit -m "refactor: promote lifecycle subcommands to root (claw up/down/ps/logs/health)"
```
...
Line 77 — confirm the command uses `claw up`:
```markdown
...
Line 81 — confirm the example uses `./bin/claw up`:
```bash
...
Lines 34 and 42 — ensure commands use `claw up`.
...
git commit -m "docs: update lifecycle command references to claw up/down/ps/logs/health"
```

---

## Task 2.5: Refactor agents.list Out of the HANDLE Platform Loop

**Files:**
- Modify: `internal/driver/openclaw/config.go:73-168`
- Modify: `internal/driver/openclaw/config_test.go`

**Bug:** `agents.list` is currently set *inside* the `case "discord":` block of the `for platform := range rc.Handles` loop. If an agent has multiple handles (Discord + Telegram), `agents.list` gets overwritten by whichever platform iterates last — silently dropping mention patterns from other platforms.

**Fix:** Extract mention pattern collection and `agents.list` generation out of the platform loop. Each platform case collects its patterns into a shared slice. After the loop, one `agents.list` entry is written with all patterns merged.

**Step 0: Add `getPath` test helper**

The existing tests use raw `json.Unmarshal` + manual type assertion. The new tests need a `getPath` helper for readability. Add this to `internal/driver/openclaw/config_test.go`:
...
```bash
go test ./internal/driver/openclaw/ -v -run TestGenerateConfigMultiPlatformMentionPatterns
```

Expected: FAIL — with current code, only one platform's patterns survive.

**Step 3: Refactor config.go**

Move mention pattern collection and `agents.list` writing out of the platform loop:

1. Before the `for platform := range rc.Handles` loop, declare:
```go
var allMentionPatterns []string
agentName := rc.ServiceName
```

2. Inside each platform case, **collect** patterns but don't write `agents.list`:
```go
case "discord":
    // ... channel config, guild config, plugin enablement (unchanged)
    // Collect mention patterns
    if h != nil {
        username := h.Username
        if username == "" {
    ...
    // REMOVE the agents.list setPath call from here
```

3. After the loop ends, write `agents.list` once with all collected patterns:
```go
// After the for platform loop closes:
if len(rc.Handles) > 0 {
...
Expected: All pass, including existing Discord tests and the new multi-platform test.

**Step 5: Commit**

```bash
git add internal/driver/openclaw/config.go internal/driver/openclaw/config_test.go
git commit -m "fix: extract agents.list from HANDLE loop to prevent multi-platform overwrite"
```
...
func detectModels(config map[string]interface{}) []string {
	...
	// Also check models.providers for cllama-rewired configs
...
> **Note:** The compose emitter publishes port 8081 automatically (see Task 8 Step 4a).
...
**Step 6a: Publish cllama dashboard port in compose emitter**

The cllama-passthrough proxy serves an operator dashboard on `:8081` but the compose emitter doesn't publish it to the host. Fix this so the dashboard is accessible at `http://localhost:8081`.

In `internal/pod/compose_emit.go`, add `Ports` to the proxy service (around line 266):
...
Also ensure `composeService` has a `Ports` field if it doesn't already:
...
Add a test in `internal/pod/compose_emit_test.go` to verify the proxy service has ports published:

```go
func TestEmitProxyPublishesDashboardPort(t *testing.T) {
    // ... emit with a proxy config, verify output contains "8081:8081"
}
```
...
curl -fsSL "$URL" -o "${TMPDIR}/${TARBALL}" || { echo "Error: download failed (HTTP error)"; exit 1; }
curl -fsSL "$CHECKSUMS_URL" -o "${TMPDIR}/checksums.txt" || { echo "Error: checksum download failed"; exit 1; }
...
The new Install section for README.md should read:
