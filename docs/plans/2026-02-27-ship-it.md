...
## Task 2.5: Refactor agents.list generation out of the HANDLE loop

**Crucial Fix:** In `internal/driver/openclaw/config.go`, the `agents.list` injection currently happens *inside* the `switch platform` block for Discord. If an agent has multiple handles (e.g. Discord + Telegram), setting `agents.list` inside the loop will overwrite the previous platform's mention patterns. Before adding Telegram and Slack, extract the `agents.list` generation so it runs *after* the `for platform := range rc.Handles` loop, aggregating `mentionPatterns` across all platforms into a single `agents.list` entry.
...
```go
	case "telegram":
		...
		// (Append these to the global mentionPatterns slice for agents.list construction later)
...
func TestInitFromOpenClawConfig(t *testing.T) {
	...
	os.WriteFile(filepath.Join(configDir, "openclaw.json"), []byte(`{
		...
		"models": {
			"providers": {
				"openrouter": {
					"models": [{"id": "anthropic/claude-sonnet-4", "name": "anthropic/claude-sonnet-4"}]
				}
			}
		}
	}`), 0644)
