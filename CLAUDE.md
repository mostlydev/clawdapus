...
- **cllama**: Standalone Go binary (in `./cllama`). OpenAI-compatible proxy on :8080, operator web UI on :8081. Multi-provider registry (OpenAI, Anthropic, OpenRouter, Ollama). Auth at `/claw/auth/providers.json` (env overrides file)
